...
if (!empty($selectedArea) && $selectedArea != '지역구를 선택해주세요') {
    // '시군구 명칭'에 부분 일치를 허용하기 위해 LIKE 연산자 사용
    $selectedArea = $conn->real_escape_string($selectedArea); // SQL 인젝션 방지
    $sql .= " AND `시군구 명칭` LIKE '%$selectedArea%'";
}

if (!empty($addressInput)) {
    $addressInput = $conn->real_escape_string($addressInput); // SQL 인젝션 방지
